...
def map_load_fmri_image(dp,target_name):
    ...
    try:
        fmri_data_clean = image.clean_img(fmri_file, detrend=True, standardize=True, confounds=confound, t_r=TR,ensure_finite=True)
    except:
...
def build_cnn3d_model_test(input_shape, Nlabels, filters=8, convsize=3, poolsize=2, hidden_size=128, conv_layers=4):
    #     import keras.backend as K
    #     if K.image_data_format() == 'channels_first':
    #         img_shape = (1,img_rows,img_cols,img_deps)
    #     elif K.image_data_format() == 'channels_last':
...
if __name__ == '__main__':
    ...
    tensorboard = TensorBoard(log_dir="logs/{}_{}cnn_win{}_lr{}_{}".format(modality,Flag_CNN_Model,block_dura,learning_rate,datetime.datetime.now().strftime("%m-%d-%H")))
    ...
    model_test_history2 = model_test.fit_generator(train_gen, epochs=100, steps_per_epoch=500,
                                                   validation_data=val_set,validation_steps=500, verbose=1,
                                                   callbacks=[checkpoint_callback,tensorboard], ##early_stopping_callback,
...
'''
class gen_data_from_lmdb(dataflow.DataFlow):
    """ Iterate through fmri filenames, confound filenames and labels
    """
    def __init__(self, lmdb_filename,ev_filename,label_matrix, target_name=None, data_type='train',train_percent=0.8):
        ###loading fmri data from lmdb and taskinfo from ev
        self.lmdb_filename = lmdb_filename
        self.ev_filename = ev_filename
        self.label_matrix = label_matrix

        if target_name is None:
            self.target_name = np.unique(self.label_matrix)
        else:
    ...
    def size(self):
        return 1e20
        #split_num=int(len(self.label_matrix)*self.train_percent)
        #if self.data_type=='train':
        #    return split_num
        #else:
        #    return len(self.label_matrix)-split_num

    def get_data(self):
        split_num=int(len(self.label_matrix)*self.train_percent)
        lmdb_env = lmdb.open(self.lmdb_filename, subdir=False)
        lmdb_txn = lmdb_env.begin()
        self.fmrifile_lists= [ key for key, _ in lmdb_txn.cursor() ]
        if self.data_type=='train':
            while True:
                rand_pos=np.random.choice(split_num,1)[0]
                yield self.lmdb_filename,self.ev_filename,rand_pos,self.fmrifile_lists
        else:
            for pos_ in range(split_num,len(label_matrix)):
                yield self.lmdb_filename,self.ev_filename,pos_,self.fmrifile_lists

def map_load_fmri_tc_atlas(dp, target_name):
    lmdb_filename = dp[0]
    ev_filename = dp[1]
    subj_id = dp[2]
    fmri_files = dp[3]

    #print('Collecting trial info from file:', ev_filename)
    label_matrix = pd.read_hdf(ev_filename, 'trials')
    sub_name = pd.read_hdf(ev_filename, 'subject')
    coding_direct = pd.read_hdf(ev_filename, 'coding')
    Subject_Num, Trial_Num = np.array(label_matrix.values).shape

    #print('Collecting fmri data from file:', lmdb_filename)
    lmdb_env = lmdb.open(lmdb_filename, subdir=False)
    lmdb_txn = lmdb_env.begin()
    if Subject_Num != len(fmri_files):
        print("Mis-matching Subjects Nums between fmri data and event design")
        #return None

    ###fmri data from lmdb file
    key = fmri_files[subj_id]
    fmri_data = loads(lmdb_txn.get(key))
    pathsub = Path(os.path.dirname(key.decode("utf-8")))
    fmri_subname = pathsub.parts[-3]
    fmri_coding = pathsub.parts[-1].split('_')[-1]
    if fmri_subname != sub_name.iloc[subj_id].values[0] or fmri_coding != coding_direct.iloc[subj_id].values[0] :
        print("Mis-matching subjects lists between fmri data and event design")
        return None
    if fmri_data is None or fmri_data.shape[0] != Trial_Num:
        print('fmri data tc length mis-matching between subjects...')
        return None

    ###mask out resting trials
    trial_label = [task.split('_')[0] for task in label_matrix.iloc[subj_id]]
    trial_mask = pd.Series(trial_label).isin(target_name)  ##['hand', 'foot','tongue']
    fmri_data_trial = fmri_data[trial_mask]
    le = preprocessing.LabelEncoder()
    le.fit(target_name)
    label_data_trial = le.transform(np.array(trial_label)[trial_mask])
    print(fmri_files[subj_id], fmri_data_cnn_test.shape, label_data_cnn_test.shape)

    return fmri_data_trial, label_data_trial

def data_pipe_fmri_tc_atlas(lmdb_filename,ev_filename,target_name=None,Region_Num=100,batch_size=32,data_type='train',train_percent=0.8,nr_thread=nr_thread,buffer_size=buffer_size):
    assert data_type in ['train', 'val', 'test']
    assert lmdb_filename is not None
    assert ev_filename is not None
    ...
    if target_name is None:
        target_name = np.unique(label_matrix)
    buffer_size = min(len(label_matrix),buffer_size)
    nr_thread = min(len(label_matrix),nr_thread)
    print('Loading data using %d threads with %d buffer_size ... \n' % (nr_thread, buffer_size))

    ds0 = gen_data_from_lmdb(lmdb_filename,ev_filename, label_matrix,data_type=data_type,train_percent=train_percent)
    print('dataflowSize is ' + str(ds0.size()))
    ####running the model
    start_time = time.clock()
    ds1 = dataflow.MultiThreadMapData(
        ds0, nr_thread=nr_thread,
        map_func=lambda dp: map_load_fmri_tc_atlas(dp,target_name),
        buffer_size=buffer_size,
        strict=True)
    ...
    ds1 = dataflow.LocallyShuffleData(ds1,buffer_size=Region_Num*buffer_size)
    ...
    for df in ds1.get_data():
        print(df[0].astype('float32').shape)
        yield (df[0].astype('float32'),to_categorical(df[1].astype('int32'),len(target_name)))
...
def data_pipe_fmri_tc_atlas(lmdb_filename,ev_filename,target_name=None,Region_Num=100,batch_size=32,data_type='train',train_percent=0.8,nr_thread=nr_thread,buffer_size=buffer_size):
    ...
    lmdb_env = lmdb.open(lmdb_filename, subdir=False)
    lmdb_txn = lmdb_env.begin()
    fmrifile_lists= [ key for key, _ in lmdb_txn.cursor() ]

    ##put both fmri data and label info into one lmdb file
    ds0 = dataflow.LMDBData(lmdb_filename, shuffle=True,keys=fmrifile_lists)
    ds1 = dataflow.LocallyShuffleData(ds0, buffer_size=Region_Num*buffer_size)
...
train_gen = data_pipe_fmri_tc_atlas(lmdb_filename,ev_filename,target_name=target_name,Region_Num=Nfeatures,
                                    batch_size=12,data_type='train',train_percent=0.8,nr_thread=2,buffer_size=2)
##out = next(train_gen)
