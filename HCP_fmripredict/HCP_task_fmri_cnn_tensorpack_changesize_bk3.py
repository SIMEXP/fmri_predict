...
def map_load_fmri_image(dp,target_name):
    ...
    fmri_data_cnn_2d = fmri_data_cnn.reshape(np.prod(fmri_data_cnn.shape[:-1]),fmri_data_cnn.shape[-1])
    fmri_data_cnn_2d = preprocessing.scale(fmri_data_cnn_2d, axis=1)
    fmri_data_cnn_2d[np.isnan(fmri_data_cnn_2d)] = 0
    fmri_data_cnn = fmri_data_cnn_2d.reshape((img_rows, img_cols, img_deps,fmri_data_cnn.shape[-1]))
...
def map_load_fmri_image_3d_block(dp, target_name,block_dura=1):
    ...
    chunks = int(np.floor(len(label_data_trial)/block_dura))
    fmri_data_cnn_block = np.array(np.array_split(fmri_data_cnn,chunks,axis=3))
    #print(chunks, np.array(fmri_data_cnn_block).shape)
    if block_dura != fmri_data_cnn_block.shape[-1]:
        print('adjust the duration of each block from %d to %d' % (block_dura,fmri_data_cnn_block.shape[-1]))
        block_dura = fmri_data_cnn_block.shape[-1]
...
def build_cnn_model_test(input_shape, Nlabels, filters=16, convsize=3, convsize2=5, poolsize=2, hidden_size=128, conv_layers=4):
    #     import keras.backend as K
    #     if K.image_data_format() == 'channels_first':
    #         img_shape = (1,img_rows,img_cols)
    #     elif K.image_data_format() == 'channels_last':
...
if __name__ == '__main__':
    ...
    model_test_history2 = model_test_GPU.fit_generator(train_gen, epochs=50, steps_per_epoch=steps,
