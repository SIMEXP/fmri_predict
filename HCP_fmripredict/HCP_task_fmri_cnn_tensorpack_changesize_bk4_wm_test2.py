...
batch_size = 2 #128  #32      ##for training of fit_generator
kfold = 2
val_size = 0.01  #0.1
#img_resize = 3
...
def map_load_fmri_image(dp,target_name):
    ...
    num_slices_sub = min(num_slices, np.sum(trial_mask))
    label_data_cnn_test = np.repeat(label_data_cnn, num_slices_sub, axis=0).flatten()  ##img_deps

    fmri_data_cnn_test = []
    for ii in range(np.sum(trial_mask)):
        rand_slice = np.random.choice(img_deps, num_slices_sub, replace=False)
        fmri_data_cnn_test.append(fmri_data_cnn[:, :, rand_slice, ii])
    fmri_data_cnn = None
    ...
    fmri_data_cnn_test = np.expand_dims(fmri_data_cnn_test, axis=-1)
...
def map_load_fmri_image_3d(dp, target_name):
    ...
    le = preprocessing.LabelEncoder()
    le.fit(target_name)
    label_data_cnn_test = le.transform(label_data_trial).flatten()  ##np_utils.to_categorical(): convert label vector to matrix
...
def data_pipe_3dcnn(fmri_files, confound_files, label_matrix, target_name=None, flag_cnn='3d', batch_size=32,
    ...
    for df in ds1.get_data():
        if np.any(np.isnan(df[0])) or not np.any(df[0]):
            print("Nan included in the image for target " + target_name(df[1]))
        else:
            if flag_cnn == '2d':
                yield (df[0].astype('float32'), df[1].astype('float32'))
            elif flag_cnn == '3d':
                yield (df[0].astype('float32'), df[1].astype('float32'))
...
def data_pipe_3dcnn_block(fmri_files, confound_files, label_matrix, target_name=None, flag_cnn='3d', block_dura=1,
    ...
    for df in ds1.get_data():
        if np.any(np.isnan(df[0])) or not np.any(df[0]):
            ...
            if flag_cnn == '2d':
                yield (df[0].astype('float32'), df[1].astype('int32'))  ##to_categorical(df[1].astype('int32'), len(target_name))
            elif flag_cnn == '3d':
                yield (df[0].astype('float32'), df[1].astype('int32'))  ##to_categorical(df[1].astype('int32'), len(target_name))
...
####################################################################
def bn(x, name='BNlayer', zero_init=False):
    return BatchNormalization(axis=1, fused=True,momentum=0.9, epsilon=1e-5,gamma_initializer='zeros' if zero_init else 'ones')(x)
...
def build_model_resnet50(image, Nlabels=8, filters=16, convsize=3, convsize2=5, poolsize=2, hidden_size=256,
    ...
    input0 = Input(shape=image) ##Input(tensor=image)
...
def build_cnn_model_test(image, Nlabels=8, filters=16, convsize=3, convsize2=5, poolsize=2, hidden_size=256, conv_layers=4):
    ...
    input0 = Input(shape=image) #Input(tensor=image)    ##use tensor instead of shape
...
if __name__ == '__main__':
    ...
    tst = data_pipe_3dcnn_block(fmri_files[:test_sub_num], confound_files[:test_sub_num], label_matrix.iloc[:test_sub_num],
                          ...
                          batch_size=2, data_type='train', nr_thread=1, buffer_size=2)
    ...
    opt_use = tf.train.AdamOptimizer(learning_rate) ##tf.train.MomentumOptimizer(lr, 0.9, use_nesterov=True)  #
    ...
    for cvi in range(kfold):
        ...
        print('training on %d subjects, validating on %d subjects ' % (len(train_sid), len(val_sid)))
        ...
        with tf.device("/cpu:0"):
            if Flag_CNN_Model == '2d':
                ...
                model_test_GPU = build_cnn_model_test(img_shape, nb_class)
            elif Flag_CNN_Model == '3d':
                print('\nTraining the model using 3d-CNN with learning-rate: %s \n' % str(learning_rate))
                model_test_GPU = build_cnn3d_model_test(img_shape, nb_class)
        #model_test_GPU = build_model_resnet50(img_shape, nb_class)

        #tf.nn.softmax_cross_entropy_with_logits
        model_test_GPU.compile(loss='sparse_categorical_crossentropy', optimizer=opt_use, metrics=['accuracy'])
        ######start training the model
        print('\nTraining the model on %d subjects and validated on %d \n' % (len(train_sid),len(val_sid)))
        model_test_history2 = model_test_GPU.fit_generator(train_gen, epochs=nepoch, steps_per_epoch=steps,verbose=1,shuffle=True)
        ...
        callbacks = [ModelSaver(checkpoint_dir=model_name),
                     ...
                     ScheduledHyperParamSetter('learning_rate', [(0, min(START_LR, BASE_LR)), (30, BASE_LR * 1e-1), (60, BASE_LR * 1e-2),(90, BASE_LR * 1e-3), (100, BASE_LR * 1e-4)]),
