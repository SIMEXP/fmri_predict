...
class hcp_task_fmri(object):
    ...
    def load_event_files(self,ev_filename=None):
        ...
        return subjects_trial_label_matrix,sub_name,coding_direct
    ...
    def prepare_fmri_files_list(self):
        ...
        self.ev_filename = 'event_labels_1200R' + '_test_' + self.subjectlist + '.h5'  # '.txt'
        self.fmri_matrix_filename = self.config.AtlasName + '_ROI_act_1200R' + '_test_' + self.subjectlist + '.lmdb'  # '.h5' # '.txt'
        self.lmdb_filename = self.config.pathout + self.modality + '_' + self.fmri_matrix_filename
        subjects_trial_label_matrix, sub_name, coding = self.load_event_files(ev_filename=self.ev_filename)

        return subjects_trial_label_matrix, sub_name, coding
...
class hcp_rsfmri(hcp_task_fmri):
    ...
    def load_fmri_data(self, sub_name=None,postfix=None,confound_name=None):
        ...
        if not startsub:
            startsub = 0
            endsub = int(round(self.config.endsub // 2 * 4))
            self.subjectlist = 'ALL'
        if (not endsub) or (endsub > len(fmri_files)):
            endsub = len(fmri_files)
        ...
        if not sub_name:
            fmri_files = fmri_files[startsub:endsub]
            confound_files = confound_files[startsub:endsub]
            WM_files = WM_files[startsub:endsub]
            CSF_files = CSF_files[startsub:endsub]
            subjects = subjects[startsub:endsub]
            ...
            print('Collecting rs-fmri files according to {} task selections of {} subjects'.format(self.config.modality,len(sub_name)))
    ...
    def prepare_rsfmri_files_list(self,sub_name=None, N_thread=None):
        ...
        if self.task_corresponding:
            ...
            corr_mat_filename = self.config.AtlasName + '_rsfc_matrix_1200R' + '_test_' + self.subjectlist + '.h5'
        if os.path.isfile(corr_mat_filename):
            corr_mat_filename = corr_mat_filename.split('.')[0] + '2.lmdb'
...
def my_svc_simple(subjects_tc_matrix,subjects_trial_label_matrix,target_name,sub_num=None, my_cv_fold=10,my_comp=20):
    ...
    if not sub_num:
        sub_num = Subject_Num
    fmri_data = np.vstack(subjects_tc_matrix[:sub_num])
    #label_data = subjects_trial_label_matrix[:Subject_Num].as_matrix().reshape(Subject_Num*Trial_Num,)
    label_data = np.array(subjects_trial_label_matrix[:sub_num]).reshape(sub_num * Trial_Num, )
    ...
    print('Time Usage of model training in seconds: {} \n'.format(time.clock() - start_time))
...
def subject_cross_validation_split(tc_matrix, label_matrix, sub_num=None, n_folds=20, testsize=0.2, valsize=0.2, randomseed=1234):
    Subject_Num, Trial_Num,Region_Num = np.array(tc_matrix).shape
    rs = np.random.RandomState(randomseed)
    if not sub_num:
        sub_num = Subject_Num
...
def build_fc_nn_subject_validation(subjects_tc_matrix,subjects_trial_label_matrix,target_name,layers=3,hidden_size=256,dropout=0.25,batch_size=128,nepochs=100,
...
class hcp_gcnn_fmri(object):
    ...
    def gccn_model_common_param(self, layers=3,hidden_size=256,pool_size=2,batch_size=128,nepochs=100):
        ...
        ##model params
        gcnn_common = {}
        gcnn_common['dir_name'] = modality + '/'
        gcnn_common['num_epochs'] = nepochs
        gcnn_common['batch_size'] = batch_size
        gcnn_common['decay_steps'] = Subject_Num / gcnn_common['batch_size']
    ...
    def build_graph_cnn_subject_validation(self,subjects_tc_matrix,subjects_trial_label_matrix,target_name,layers=3,hidden_size=256,pool_size=2,batch_size=128,nepochs=100,
                                       my_cv_fold=10,testsize=0.2,valsize=0.2):
        ...
        ##split data into train, val and test in subject-level
        X_train, Y_train, X_val, Y_val, X_test, Y_test = subject_cross_validation_split(subjects_tc_matrix,
                                                                                        subjects_trial_label_matrix,
                                                                                        n_folds=my_cv_fold,
                                                                                        testsize=testsize,
                                                                                        valsize=valsize)
        print(np.array(X_train).shape,np.array(X_test).shape,np.array(X_val).shape)
        print('Samples for training: %d and testing: %d with %d features' % (np.array(X_train).shape[1], np.array(X_test).shape[0], np.array(X_train).shape[2]))
        ...
        self.gccn_model_common_param(layers=layers, hidden_size=hidden_size, pool_size=pool_size, batch_size=batch_size, nepochs=nepochs)
        common = self.gcnn_common
